...
from ..models import MatchResult, MatchSession

SERVER_BASE_URL = settings.AI_URL


def _proxy_request_to_main_server(
    endpoint, request_method, request_data=None, request_files=None
):
    ...
    try:
        ...
        return Response(
            {"error": f"Lỗi giao tiếp với server chính: {e}"},
...
class ResumeViewSet(viewsets.ViewSet):
    ...
    def upload_resume(self, request):
        if "file" not in request.FILES:
            return Response(
                {"error": "Không tìm thấy file."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return _proxy_request_to_main_server(
            "/api/resumes/upload/", "POST", request_files=files
        )
...
class JobDescriptionViewSet(viewsets.ViewSet):
    ...
    def upload_jd(self, request):
        ...
        return _proxy_request_to_main_server(
            "/api/jobdescriptions/upload/", "POST", request_files=files
        )
    ...
    def delete_jd(self, request, pk=None):
        return _proxy_request_to_main_server(
            f"/api/jobdescriptions/{pk}/delete/", "DELETE"
        )
...
class ProcessWithAI(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Yêu cầu đến server AI bị timeout."},
                status=status.HTTP_504_GATEWAY_TIMEOUT,
            )
        except requests.exceptions.RequestException as e:
            return Response(
                {"error": f"Lỗi giao tiếp với server AI: {e}"},
                status=status.HTTP_502_BAD_GATEWAY,
            )
        except ValueError:
            return Response(
                {"error": "Server AI trả về dữ liệu không hợp lệ."},
        ...
        try:
            ...
            if not jd_id:
                return Response(
                    {
                        "error": f"'{jd_id}' - {request_data} is missing from the request body."
                    },
